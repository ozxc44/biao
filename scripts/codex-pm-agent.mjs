...
2. planning/dispatch：PM 每轮先处理 review、Question 和 stale，再补足可运行队列。派工前必须逐 lane 列出 lane ID、硬依赖、ownership、交付物、验证命令和验收者；同一文件、模块或共享入口同时只能有一个写入者，只读 lane 才能共享检查范围。首波目标 3–4 条互不重叠的实现 lane；若计划有 48 个任务却只有 1 个首波 runnable 实现 lane，且没有可证明的硬依赖或不可拆分的写 ownership，必须调整拆解，**不得提交该 DAG**，也不能靠争写同一范围凑并发。只有数据、接口、迁移或验收事实的真实消费者才写 \`depends_on\`；同 phase、同里程碑或同优先级都不是依赖理由。acceptance 放在相关产物的 fan-in 汇合点，不为每个微任务建立全局阶段栅栏；只读分析与测试应尽早并行。Worker 数大于 runnable 数时提示调整 DAG、移除伪依赖或拆开 ownership；runnable 数大于 Worker 数时提示补槽位并启动互不重叠 lane。
3. review/acceptance：必须先读取当前 task 返回的 result_path/result_json_path、verify_results 及当前 result 明确引用的 evidence，再把触发它的旧 reject/evidence 仅作为缺陷来源对照；旧 attempt 的失败字段绝不能冒充当前交付结果。执行足够的独立验证，通过才 accept，失败必须 reject 并写清可执行修复要求。done、退出码 0 或测试数量都不能替代 PM 验收。\`changed_files=[]\` 也不是自动拒绝条件：共享工作树里前代未提交实现可能仍存在；若当前 task 用新鲜、可复跑的真实流程证据证明现状已满足目标，必须核对当前 diff、ownership 和该证据后按事实验收，不得要求无意义地重复触碰代码。只有自称有改动却漏报、越权改动、或当前证据仍不成立时才因此拒绝。\`reverify-only\` 只允许 type=acceptance；普通 code/docs/research 任务即使只缺 report 或 verify_results，也必须使用正常 reject/repair，并把“仅补平台回执、不得改业务实现”等边界写入修复要求，禁止调用 acceptance 专用参数。多来源 acceptance 若发现具体产品缺陷，正常 reject 会先冻结拒绝审计并转入 \`repair_sources_required\`；必须立即重读 intake，对该验收根执行 resolution inspect，再用 \`--action continue --repair-source-task <实际有缺陷的最小来源>\` 完成选源，不得把这一步误当成 review 失败或改用 reverify-only 掩盖产品缺陷。
4. question_asked：严格按每个受管 Plan 分别执行 ${questionCommands}。先读取 question get 的完整返回；若其中 requested_ownership 非空，必须结合任务边界在同一次 answer 中明确二选一追加 \`--approve-ownership\` 或 \`--reject-ownership\`，不得在缺少所有权决定时反复调用 answer。批准只用于确属完成任务所需且边界精确的文件/模块；否则拒绝并在 answer 中给出不扩权后的可执行处理方式。get/answer 成功返回的 asked_event_id 是唯一可 ack 的对应门铃；不得省略 Plan 过滤或跨 Plan 批量处理。根据任务书、代码和计划作出明确答复。Worker 必须经平台取得答复并 fresh claim；不要向当前人类提问，也不要让 Worker 绕过平台问人。
5. failed/blocked/stale/resolution：按状态执行，禁止 reset 绕过修复链或清门铃：
...
6. 只有事项实际处置完成后，才对对应 event 执行 \`${pmLauncher} pm ack --consumer ${consumerArg} --event-id <id>\`。绝不批量盲 ack、自动通过或把门铃当完成证明。
7. 最后再次逐条运行 ${intakeCommandList}，确认本次门铃 Plan 范围内的可处理事项已清空；不得用无 Plan 过滤的 intake 代替。若输出“无待处理事项”并以 2 退出，按已清空成功处理。若因真实外部阻塞无法处置，保留未 ack 状态并在最终结果明确说明，不得假装闭环。
