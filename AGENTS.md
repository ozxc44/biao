...
- `.biao/pm-watch` is the low-resource resident doorbell watcher for a central Biao (no local server, no worker slots, stay-resident). Set `BIAO_SUPERVISOR_AUTO_ENSURE=1` in `.biao/config.env` to self-heal: after a successful `task_report` (MCP or worker runtime `/report`), `pm ack`, or `question answer`, the machine re-ensures `pm-watch` is running via `pm-watch --ensure` (idempotent, single-instance locked). Useful on every worker/PM machine that talks to a shared central Biao.
